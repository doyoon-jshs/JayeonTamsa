...
cs = plt.contour(masked_depth, levels=np.arange(np.nanmin(masked_depth), np.nanmax(masked_depth), 5), cmap=cm.cool)
...
plt.show()
from mpl_toolkits.mplot3d import Axes3D  # 꼭 import 해주세요
...
# Prepare grid
X, Y = np.meshgrid(np.arange(masked_depth.shape[1]), np.arange(masked_depth.shape[0]))
Z = masked_depth.copy()
...
ax.set_title("3D Depth Surface (cm)")
ax.set_xlabel("X")
ax.set_ylabel("Y")
ax.set_zlabel("Depth (cm)")

# Colorbar
fig.colorbar(surf, shrink=0.5, aspect=10, label="Depth (cm)")

plt.tight_layout()
plt.savefig("depth_surface_3d.png", dpi=300)
plt.show()
